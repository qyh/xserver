...
for tx in title:
	...
	idx = idx + 1
row = 2
start_time_str = "2017-12-31"
end_time_str = "2019-01-01"
...
while tmp_time <= e_time:
	...
	# 每张表查询牌局 top 10
	sql = 'SELECT date_format(starttime, "%Y-%m-%d") as d, userid, nickname, count(*) as s FROM (select userid,startTime,nickname from oss_gamelog{}.gameuserinfolog_{} where startTime>=\'2018-01-01\' and starttime < \'{}\' union select userid,startTime,nickname from oss_gamelog{}.matchuserinfolog_{} where startTime>=\'2018-01-01\' and starttime <\'{}\' union select userid,startTime,nickname from oss_gamelog{}.newroomselfuserinfolog_{} where  startTime>=\'2018-01-01\' and starttime <\'{}\') group by date_format(starttime, "%Y-%m-%d"), userid, nickname order by s desc  limit 10;'.format(year,_str, end_time_str,year,_str, end_time_str,year,_str,end_time_str)
	print("sql:", sql)
	cursor.execute(sql)
	# 获取记录并放入topList
