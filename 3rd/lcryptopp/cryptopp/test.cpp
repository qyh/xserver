...
int CRYPTOPP_API main(int argc, char *argv[])
{
	...
	{		
		...
		if (command == "g")
		{
			...
			void (*gen)(unsigned int keyLength, const char *privFilename, const char *pubFilename, const char *seed);

			cout << "RSA encryption scheme(OAEP or not): ";
			cin >> OAEP;
			...
			gen = OAEP ? GenerateRSAKey<RSAES_OAEP_SHA_Decryptor, RSAES_OAEP_SHA_Encryptor> :
				GenerateRSAKey<RSAES_PKCS1v15_Decryptor, RSAES_PKCS1v15_Encryptor>;
			gen(keyLength, privFilename, pubFilename, thisSeed);
		}
		else if (command == "rs")
		...
		else if (command == "r")
		{
			...
			string (*enc) (const char *pubFilename, const char *seed, const char *message);
			string (*dec)(const char *privFilename, const char *ciphertext);
			...
			enc = OAEP ? RSAEncryptString<RSAES_OAEP_SHA_Encryptor> : RSAEncryptString<RSAES_PKCS1v15_Encryptor>;
			dec = OAEP ? RSADecryptString<RSAES_OAEP_SHA_Decryptor> : RSADecryptString<RSAES_PKCS1v15_Decryptor>;

			string ciphertext = enc(pubFilename, thisSeed, message);
			cout << "\nCiphertext: " << ciphertext << endl;

			string decrypted = dec(privFilename, ciphertext.c_str());
			cout << "\nDecrypted: " << decrypted << endl;
		}
		else if (command == "rd")
		{
			bool OAEP = false;
			char privFilename[128];
			...
			dec = OAEP ? RSADecryptString<RSAES_OAEP_SHA_Decryptor> : RSADecryptString<RSAES_PKCS1v15_Decryptor>;
			string decrypted = dec(privFilename, ciphertext);
			cout << "\nDecrypted: " << decrypted << endl;
		}
		else if (command == "mt")
		{
...
void GenerateRSAKey(unsigned int keyLength, const char *privFilename, const char *pubFilename, const char *seed)
{
	...
	Decryptor priv(randPool, keyLength);
